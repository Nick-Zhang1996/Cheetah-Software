 ...
 */
template <typename T>
FSM_State_NickJump<T>::FSM_State_NickJump(ControlFSMData<T>* _controlFSMData)
    : FSM_State<T>(_controlFSMData, FSM_StateName::NICKJUMP, "Nick_Jump"){
...
template <typename T>
void FSM_State_NickJump<T>::onEnter() {
...
template <typename T>
bool FSM_State_NickJump<T>::_Initialization() { // do away with this?
...
template <typename T>
void FSM_State_NickJump<T>::ComputeCommand() {
  if (_b_first_visit) {
  ...
  if(this->_data->controlParameters->use_rc){
    // not designed to work with rc control
...
template <typename T>
void FSM_State_NickJump<T>::_SafeCommand() {
  for (int leg = 0; leg < 4; ++leg) {
    for (int jidx = 0; jidx < 3; ++jidx) {
      this->_data->_legController->commands[leg].tauFeedForward[jidx] = 0.;
...
template <typename T>
void FSM_State_NickJump<T>::_SetJPosInterPts(
 ...
 */
template <typename T>
FSM_StateName FSM_State_NickJump<T>::checkTransition() {
  this->nextStateName = this->stateName;
  iter++;
 ...
 */
template <typename T>
TransitionData<T> FSM_State_NickJump<T>::transition() {
  // Finish Transition
 ...
 */
template <typename T>
void FSM_State_NickJump<T>::onExit() {
  // nothing to clean up?
}

// template class FSM_State_NickJump<double>;
template class FSM_State_NickJump<float>;
